...
export async function collectFilesWithPathsRecursively(params: {
  ...
  pathParts: string[]
  items: Array<{ file: File; path: string; name: string; size: number }>
}) {
  ...
  for await (const entry of directoryHandle.values()) {
    ...
    if (isFileHandle(entry)) {
      ...
      items.push({ file, path: relFromRoot, name: file?.name ?? '', size: file?.size ?? 0 })
      continue
    }
...
export async function pickDirectoryFilesWithPaths(): Promise<Array<{ file: File; path: string; name: string; size: number }>> {
...
export function indexFilesWithPath(params: { files: File[] }) {
  ...
  const indexed = files.map((file) => {
    ...
    const entry = { file, path, name: file?.name ?? '', size: file?.size ?? 0 }
    return entry
  })
  return indexed
}
